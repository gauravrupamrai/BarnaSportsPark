...
async function getCourtAvailability(body) {
  try {
    ...
    for (let court in bookedSlots) {
      availableSlots[court] = allSlots.filter(
        (slot) => !bookedSlots[court].includes(slot)
      );
    }

    return util.buildResponse(200, "Court Availability", {
      bookedSlots,
      availableSlots,
    });
  } catch (error) {
...
async function getBookings(body) {
  try {
    ...
    const pastBookings = bookings.filter(booking => booking.bookingDate < currentDate);

    const upcomingBookings = bookings.filter(booking => booking.bookingDate >= currentDate);

    console.log("pastBookings: ", pastBookings);
    console.log("upcomingBookings: ", upcomingBookings);
    
    return util.buildResponse(200, "Bookings fetched successfully", {
      pastBookings,
      upcomingBookings,
    });
  } catch (error) {
